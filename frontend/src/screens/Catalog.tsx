  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Select label="Overfit filtresi" value={overfitFilter} options={["Tümü", "low", "mid", "high"]} onChange={setOverfitFilter} />
          <Select label="Kaynak" value={sourceFilter} options={sources} hint="evolution · llm · mcts" onChange={setSourceFilter} />
          <Check label="Sadece backtest yapılmışlar" checked={onlyBacktest} onChange={setOnlyBacktest} />
          <input
            value={regex}
        ...
        {/* Table — scrollable, flex:1 */}
        <div style={{ flex: 1, minHeight: 0, overflow: "auto" }}>
          {isLoading ? (
            <div style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--fg-3)", padding: 16 }}>Yükleniyor…</div>
          ) : (
            <>
              {/* Header */}
              <div
              ...
              >
                <span /><span>#</span><span>formül</span>
                ...
                <span>overfit</span><span>best</span><span>kaynak</span>
              </div>
              ...
              {filtered.map((r: CatalogRecord, i: number) => {
                ...
                return (
                  ...
                  >
                    ...
                    <code style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--fg-0)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                      {r.formula}
                    </code>
                    <span style={{ fontFamily: "var(--mono)", textAlign: "right", color: "var(--fg-1)" }}>{r.ic != null ? r.ic.toFixed(4) : "—"}</span>
                    <span style={{ fontFamily: "var(--mono)", textAlign: "right", color: (r.rank_ic ?? 0) > 0.02 ? "var(--pos)" : "var(--fg-1)" }}>{r.rank_ic != null ? r.rank_ic.toFixed(4) : "—"}</span>
                    <span style={{ fontFamily: "var(--mono)", textAlign: "right", color: "var(--fg-2)" }}>{r.adj_ic != null ? r.adj_ic.toFixed(4) : "—"}</span>
                    <span style={{ fontFamily: "var(--mono)", fontSize: 10.5, color: "var(--fg-1)" }}>{r.wf_score != null ? `fit=${r.wf_score.toFixed(4)}` : "—"}</span>
                    <span style={{ fontFamily: "var(--mono)", textAlign: "right", color: r.sharpe != null ? "var(--pos)" : "var(--fg-3)" }}>{r.sharpe != null ? r.sharpe.toFixed(2) : "—"}</span>
                    <span style={{ fontFamily: "var(--mono)", fontSize: 10, color: r.overfit_score != null && r.overfit_score < 0.3 ? "var(--pos)" : r.overfit_score != null && r.overfit_score < 0.6 ? "var(--warn)" : "var(--fg-3)" }}>
                      {r.overfit_score != null ? (r.overfit_score < 0.3 ? "low" : r.overfit_score < 0.6 ? "mid" : "high") : "—"}
                    </span>
                    <span style={{ fontFamily: "var(--mono)", fontSize: 10, color: "var(--fg-3)" }}>{r.best_window ?? "—"}</span>
                    <span style={{ fontFamily: "var(--mono)", fontSize: 10, color: "var(--fg-2)" }}>{r.source ?? "—"}</span>
                  </div>
                );
              })}
            </>
        ...
        {/* Footer actions — sabit alt bar */}
        <div style={{ borderTop: "1px solid var(--line-soft)", background: "var(--bg-1)" }}>
          {selected.size > 0 && (
            <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "8px 18px", borderBottom: "1px solid var(--line-soft)" }}>
              <span style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--fg-1)", flex: 1 }}>{selected.size} kayıt seçili</span>
              <Btn variant="danger" onClick={handleDeleteSelected}>✕ Seçilenleri sil</Btn>
            </div>
          )}
          <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "8px 18px" }}>
            <span style={{ fontFamily: "var(--mono)", fontSize: 10, color: "var(--warn)" }}>⚠</span>
            <span style={{ fontSize: 11.5, color: "var(--fg-1)", flex: 1 }}>Katalogdaki TÜM kayıtları sil
              <span style={{ fontFamily: "var(--mono)", fontSize: 9.5, color: "var(--fg-3)", marginLeft: 8 }}>sadece JSON — buffer korunur</span>
            </span>
